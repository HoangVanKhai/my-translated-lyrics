...
- Use descriptive names for variables and closure parameters. Single letters are permitted only in these cases: conventional names like `n` for count or `f` for formatter, comparison closures like `|a, b|`, trivial single-expression closures, fold accumulators, index variables `i`/`j`/`k` in short closures or index-based loops, and test fixtures with identical roles. Single letters are never permitted in multi-line functions or closures.
