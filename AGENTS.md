...
- Write documentation, comments, and other prose in a direct, formal tone using complete sentences. Avoid mid-sentence breaks introduced by em dashes, long parentheticals, or conversational asides such as "basically", "just", "that'd be slow", or "etc." Em dashes are a reliable symptom of loose phrasing; when one appears, restructure the surrounding sentence so each clause stands on its own rather than swapping the em dash for another punctuation mark.
