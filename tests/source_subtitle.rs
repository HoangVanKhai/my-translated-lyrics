...
/// `credit-roles` and `credit-names` are in natural sorted order, and whose
/// on-disk text matches the canonical [`serde_norway`] serialization of the
/// parsed value (so the file can be regenerated by an automated formatter
/// without a diff).
#[test]
fn source_subtitle_descriptors() {
    ...
    for entry in entries {
        ...
        let desc: SubtitleDesc = original.pipe_as_ref(serde_norway::from_str).unwrap();
