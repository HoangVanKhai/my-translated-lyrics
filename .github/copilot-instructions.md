...
- Run `cargo fmt -- --check && cargo clippy && cargo test` to validate changes
- **ALWAYS run the full test suite** (`cargo fmt -- --check && cargo clippy && cargo test`) before committing, regardless of how trivial the change seems — this includes documentation-only changes, comment edits, and config changes
