...
- **Let the tail be a cursor when the text alone is not enough.** A grammar read line by line needs more than the remaining characters: the parser that reports a diagnostic needs the line number, and the parser reading the inside of an `<additive>` region needs to know that it is there. The lyrics parser therefore threads a `Cursor` value that carries the unread text along with both, and its parsers return `Result<(T, Cursor), ParseLyricsError>`. The rule is the same either way: a parser hands back the position the next one starts from, and the caller never recomputes that position from the length of what was matched.
